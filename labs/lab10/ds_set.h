...
private:
  ...
  TreeNode<T>* copy_tree(TreeNode<T>* old_root) {
    ...
    // return if old_root is NULL
    if (!old_root) return NULL;

    // create a new node with the same value as old_root
...
// find using iterative approach
  iterator find(const T& key_value, TreeNode<T>* p) {
    while (p) {
      // if key_value is less than current node's value, go left
      if (p->value > key_value)
        p = p->left;
      // if key_value is greater than current node's value, go right
