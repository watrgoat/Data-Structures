...
void dslist<T>::push_front(const T& v) {
  ...
  if (!head_) {
    ...
    // otherwise add new node before current head
    newp->next_ = head_;
    head_->prev_ = newp;
    head_ = newp;
  }
...
void dslist<T>::pop_front() {
  if (!head_) return; // ignore empty list case
  if (head_ == tail_) { // only one node in list
    delete head_;
    head_ = NULL;
    tail_ = NULL;
  } else { // otherwise remove first node
    head_ = head_->next_;
    delete head_->prev_;
    head_->prev_ = NULL;
  }
...
void dslist<T>::pop_back() {
  if (!tail_) return; // ignore empty list case
  if (head_ == tail_) { // only one node in list
    delete tail_;
    head_ = NULL;
    tail_ = NULL;
  } else { // otherwise remove last node
    tail_ = tail_->prev_;
    delete tail_->next_;
    tail_->next_ = NULL;
  }
