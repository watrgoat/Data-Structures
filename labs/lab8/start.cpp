...
void print_grid_line(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
                unsigned int start_x, unsigned int start_y, vector<Point> path) {

  std::cout << "Here is the grid with the origin in the upper left corner, x increasing \n"
            << "horizontally and y increasing down the screen.  An 'X' represents a blocked\n"
            << "location and the 'S' represents the starting location. '$' is found path.\n\n";
  
  for (unsigned int y=0; y<blocked_grid[0].size(); ++y) {
    for (unsigned int x=0; x<blocked_grid.size(); ++x) {
      // if value is in path, print $
...
int solve(const vector<vector<GRID_STATUS>> &grid, int start_x, int start_y) {
  // traverse grid only in directions left and up
  // if blocked, return 0
  // if at origin, return 1
  // else return sum of left and up
  if (start_x==0 && start_y==0) {
    return 1;
  }
  int sum = 0;
  if (start_x > 0 && grid[start_x-1][start_y] != GRID_BLOCKED) {
    sum += solve(grid, start_x-1, start_y);
  }
  if (start_y > 0 && grid[start_x][start_y-1] != GRID_BLOCKED) {
    sum += solve(grid, start_x, start_y-1);
  }
  return sum;
}

vector<Point> solveLine(vector<vector<GRID_STATUS>> &grid, int x, int y) {
  // traverse grid creating one path to origin
  
  if (x==0 && y==0) {
    return {Point(0, 0)};
  }
  
  if (x < 0 || y < 0 || grid[x][y] == GRID_BLOCKED) {
    return {};
  }

  if (x > 0) {
    vector<Point> leftPath = solveLine(grid, x-1, y);
    if (!leftPath.empty()) {
      leftPath.push_back(Point(x, y));
      return leftPath;
    }
  }

  if (y > 0) {
    vector<Point> upPath = solveLine(grid, x, y-1);
    if (!upPath.empty()) {
      upPath.push_back(Point(x, y));
      return upPath;
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  int paths = solve(blocked_grid, start_x, start_y);
  cout << "num paths: " << paths << endl;

  if (paths > 0) {
    vector<Point> path = solveLine(blocked_grid, start_x, start_y);
    for (int i = 0; i < path.size(); i++) {
      cout << path[i].x << " " << path[i].y << endl;
    }
    // write the points onto the grid
    print_grid_line(blocked_grid, start_x, start_y, path);
  }
