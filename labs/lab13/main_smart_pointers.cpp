...
// use set of visited pointers to avoid infinite recursion
// look through all the balloons we are attached to, and all the balloons

void deallocate(Balloon* b, std::set<Balloon*>& visited) {
  if (visited.find(b) != visited.end()) return;
  visited.insert(b);
  for (int i = 0; i < b->numRopes(); i++) {
    deallocate(b->getRope(i), visited);
  }
  delete b;
}

void deleteAll(Balloon* b) {
  std::set<Balloon*> visited;
  deallocate(b, visited);
}
